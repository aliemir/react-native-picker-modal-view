...
export const generateAlphabet = (
    items: IModalListInDto[],
    sortingLanguage?: string
): Array<string> => {
    const singularAlpha: IModalListInDto[] = [];
    ...
    if (sortingLanguage === "tr") {
        ...
        return singularAlpha.sort((a, b) => (a as string).localeCompare(b));
    }
};

// source https://gist.github.com/ugurozpinar/9682734
export const trCompare = (a: string, b: string): number => {
    const alphabets =
...
export const getFilteredData = (
    ...
    return items.filter(
        (l: IModalListInDto) =>
            l.Name.toLocaleLowerCase().indexOf(searchText.toLocaleLowerCase()) >
            -1
    );
};
